    ...
    public Mono<Void> filterBlackList(ServerWebExchange exchange) {
        ...
        try {
            ...
            if (originUri != null) {
                ...
                if (forbid.get()) {
                    ResultEntity<String> accessDenied = ResultEntity.accessDenied("黑名单限制，禁止访问");
                    DataBuffer dataBuffer = response.bufferFactory().wrap(JSONObject.toJSONString(accessDenied).getBytes());
    ...
    private Mono<Void> doRateLimitCheck(AtomicBoolean limit, RateLimitRule rule, URI uri,
        ...
        if (limit.get()) {
            ...
            if (count == 0) {
                ...
                ResultEntity<String> accessDenied = ResultEntity.accessDenied("访问频率超限，请稍后再试");
                DataBuffer dataBuffer = response.bufferFactory().wrap(JSONObject.toJSONString(accessDenied).getBytes());
                return response.writeWith(Mono.just(dataBuffer));
            } else {
