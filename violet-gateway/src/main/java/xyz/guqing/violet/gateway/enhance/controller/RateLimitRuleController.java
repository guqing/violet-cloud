...
public class RateLimitRuleController {
    ...
    @GetMapping("data")
    public Flux<RateLimitRule> listByPages(PageQuery request, RateLimitRuleQuery rateLimitRuleQuery) {
        RateLimitRule rateLimitRule = rateLimitRuleQuery.convertTo();
        return rateLimitRuleService.findPages(request, rateLimitRule);
    }

    @GetMapping("count")
    public Mono<Long> countRateLimitRules(RateLimitRuleQuery rateLimitRuleQuery) {
        RateLimitRule rateLimitRule = rateLimitRuleQuery.convertTo();
        return rateLimitRuleService.findCount(rateLimitRule);
    }

    @GetMapping("exist")
    public Flux<RateLimitRule> listBy(String requestUri, String requestMethod) {
